 ...
 *
 * <p>Virtual threads in Java 21 work excellently with ScopedValue for context propagation. Unlike
 * ThreadLocal, ScopedValue is designed for virtual threads and provides: - Automatic inheritance to
 * child virtual threads - Better performance (no copying of values) - Immutability guarantees
 * within scope
 */
public class Executors {

  /**
   * Virtual thread executor for general processing tasks. Each task runs in its own virtual thread.
   */
  ...
  /**
   * Executes a task in a virtual thread with the specified match ID in scope. The match ID will be
   * available via ProcessContext.MATCH_ID.get() within the task.
   *
   * @param matchId the match ID to set in scope
   * @param task the task to execute
   */
  public static void executeWithMatchId(Long matchId, Runnable task) {
    PROCESSOR_VIRT_EXECUTOR.execute(() -> ProcessContext.runWithMatchId(matchId, task));
  }

  /**
   * Executes a task in a virtual thread with the specified player ID in scope. The player ID will
   * be available via ProcessContext.PLAYER_ID.get() within the task.
   *
   * @param playerId the player ID to set in scope
   * @param task the task to execute
   */
  public static void executeWithPlayerId(Long playerId, Runnable task) {
    PROCESSOR_VIRT_EXECUTOR.execute(() -> ProcessContext.runWithPlayerId(playerId, task));
  }
  ...
  public static void executeWithContext(Long matchId, Long playerId, Runnable task) {
    PROCESSOR_VIRT_EXECUTOR.execute(() -> ProcessContext.runWithContext(matchId, playerId, task));
  }
   ...
   * @param task the task to execute
   */
  public static void executeWithFullContext(
